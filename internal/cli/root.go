...
func Run(ctx context.Context, stdout, stderr io.Writer, args []string, baseURL string) int {
	...
	if len(args) == 0 {
		_, _ = fmt.Fprintln(stderr, "usage: anchorctl <repo|anchor|context|comment|search|version> ...")
		return 1
	}
	client := &http.Client{}
	switch args[0] {
	...
	case "version", "--version", "-version":
		_, _ = fmt.Fprintf(stdout, "anchorctl %s\n", version.String())
		return 0
	default:
...
func runAnchor(ctx context.Context, client *http.Client, stdout, stderr io.Writer, args []string, baseURL string) int {
	if len(args) == 0 {
		_, _ = fmt.Fprintln(stderr, "usage: anchorctl anchor <list|get|create|update|close|reopen|resolve|events|stale|candidates|relocate>")
		return 1
	}
	switch args[0] {
	case "list":
	...
	case "events":
		fs := flag.NewFlagSet("anchor events", flag.ContinueOnError)
		fs.SetOutput(stderr)
		id := fs.String("id", "", "")
		if err := fs.Parse(args[1:]); err != nil {
			return 1
		}
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, baseURL+"/v1/anchors/"+*id+"/events", nil)
		if err != nil {
			_, _ = fmt.Fprintln(stderr, err)
			return 1
		}
		return doRequest(client, stdout, stderr, req)
	case "stale":
		fs := flag.NewFlagSet("anchor stale", flag.ContinueOnError)
		fs.SetOutput(stderr)
		repoID := fs.String("repo-id", "", "")
		limit := fs.Int("limit", 0, "")
		if err := fs.Parse(args[1:]); err != nil {
		...
		return doRequest(client, stdout, stderr, req)
	case "candidates":
		fs := flag.NewFlagSet("anchor candidates", flag.ContinueOnError)
		fs.SetOutput(stderr)
		id := fs.String("id", "", "")
		if err := fs.Parse(args[1:]); err != nil {
			return 1
		}
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, baseURL+"/v1/anchors/"+*id+"/candidates", nil)
		if err != nil {
			_, _ = fmt.Fprintln(stderr, err)
			return 1
		}
		return doRequest(client, stdout, stderr, req)
	case "relocate":
		fs := flag.NewFlagSet("anchor relocate", flag.ContinueOnError)
		fs.SetOutput(stderr)
		id := fs.String("id", "", "")
		candidate := fs.Int("candidate", -1, "")
		startLine := fs.Int("start-line", 0, "")
		startCol := fs.Int("start-col", 1, "")
		endLine := fs.Int("end-line", 0, "")
		endCol := fs.Int("end-col", 1, "")
		...
		case *startLine > 0:
			...
			payload["start_col"] = *startCol
			payload["end_line"] = *endLine
			payload["end_col"] = *endCol
		default:
			_, _ = fmt.Fprintln(stderr, "provide -candidate <n> or -start-line/-end-line")
			return 1
		}
		return doJSONRequest(ctx, client, stdout, stderr, http.MethodPost, baseURL+"/v1/anchors/"+*id+"/relocate", payload)
