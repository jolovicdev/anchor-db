...
func (s *Store) CreateAnchor(ctx context.Context, anchor domain.Anchor) (domain.Anchor, error) {
	...
	err = s.withTx(ctx, func(tx *sql.Tx) error {
		...
		if err := addEvent(ctx, tx, domain.AnchorEvent{
		...
		return upsertAnchorSearch(ctx, tx, anchor)
	})
	if err != nil {
		return domain.Anchor{}, err
	}
...
func (s *Store) UpdateAnchor(ctx context.Context, anchor domain.Anchor, reason string) (domain.Anchor, error) {
	...
	err := s.withTx(ctx, func(tx *sql.Tx) error {
		previous, err := getAnchor(ctx, tx, anchor.ID)
		if err != nil {
			return err
		}
		updated, err = updateAnchorRecord(ctx, tx, anchor)
		if err != nil {
			return err
		}
		return addEvent(ctx, tx, domain.AnchorEvent{
			ID:          domain.NewID("event"),
			AnchorID:    updated.ID,
...
func updateAnchorRecord(ctx context.Context, ex executor, anchor domain.Anchor) (domain.Anchor, error) {
	...
	// Without this the update silently succeeds for an unknown ID, leaving the
	// caller with an anchor that was never persisted.
	...
	if err := reindexCommentsForAnchor(ctx, ex, anchor); err != nil {
		return domain.Anchor{}, err
	}
	return anchor, nil
}
...
func (s *Store) GetAnchor(ctx context.Context, id string) (domain.Anchor, error) {
	return getAnchor(ctx, s.db, id)
}

func getAnchor(ctx context.Context, ex executor, id string) (domain.Anchor, error) {
...
func (s *Store) ListAnchors(ctx context.Context, filter domain.AnchorFilter) ([]domain.Anchor, error) {
	return listAnchors(ctx, s.db, filter)
}

func listAnchors(ctx context.Context, ex executor, filter domain.AnchorFilter) ([]domain.Anchor, error) {
...
func (s *Store) ApplyResolution(ctx context.Context, anchorID string, binding domain.Binding, status domain.AnchorStatus, reason string, confidence float64) (domain.Anchor, error) {
	var updated domain.Anchor
	err := s.withTx(ctx, func(tx *sql.Tx) error {
		anchor, err := getAnchor(ctx, tx, anchorID)
		if err != nil {
			return err
		}
		...
		// Resolution runs on every sync pass. Recording an event when nothing
		// actually changed would grow the history without bound, so an
		// unchanged binding and status is left unrecorded.
		unchanged := previousStatus == status && bindingEqual(previousBinding, binding)
		if unchanged {
			return nil
		}
...
func addEvent(ctx context.Context, ex executor, event domain.AnchorEvent) error {
	var fromJSON any
	var toJSON any
