...
type Service interface {
	...
	StaleQueue(context.Context, string, int) ([]app.StaleEntry, error)
	RelocationCandidates(context.Context, string) ([]app.RelocationCandidate, error)
	AcceptRelocation(context.Context, app.RelocateInput) (domain.Anchor, error)
...
func New(service Service) *mcp.Server {
	...
	mcp.AddTool(server, &mcp.Tool{
		Name:        "anchor_events",
		Description: "Read an anchor's history: when it moved, went stale, or was updated, with the reason and confidence for each step.",
	}, api.anchorEvents)

	mcp.AddTool(server, &mcp.Tool{
		Name:        "anchor_stale",
		Description: "List anchors that could not be relocated automatically, each with its best relocation suggestion. Use this to find notes that need re-pinning after a refactor.",
	}, api.anchorStale)

	mcp.AddTool(server, &mcp.Tool{
		Name:        "anchor_candidates",
		Description: "Rank the places a stale anchor might now belong, with confidence and a code preview for each.",
	}, api.anchorCandidates)
	...
	server.AddResourceTemplate(&mcp.ResourceTemplate{
		...
		Description: "Move, stale, and update history for a single AnchorDB anchor.",
		MIMEType:    "application/json",
		URITemplate: "anchordb://events/{anchor_id}",
	}, api.readEvents)
...
type eventsOutput struct {
	Events []domain.AnchorEvent `json:"events"`
}

type staleInput struct {
	RepoID string `json:"repo_id,omitempty" jsonschema:"Optional repo ID to scope the queue."`
	Limit  int    `json:"limit,omitempty" jsonschema:"Optional maximum number of anchors to return."`
}

type staleOutput struct {
	Stale []app.StaleEntry `json:"stale"`
}

type candidatesOutput struct {
	Candidates []app.RelocationCandidate `json:"candidates"`
}
...
func (a *API) repoAdd(ctx context.Context, _ *mcp.CallToolRequest, input repoCreateInput) (*mcp.CallToolResult, domain.Repo, error) {
	repo, err := a.service.RegisterRepo(ctx, input.Name, input.Path)
	return nil, repo, err
}

func (a *API) anchorContext(ctx context.Context, _ *mcp.CallToolRequest, input contextInput) (*mcp.CallToolResult, app.ContextResponse, error) {
...
func (a *API) anchorRepos(ctx context.Context, _ *mcp.CallToolRequest, _ struct{}) (*mcp.CallToolResult, reposOutput, error) {
	repos, err := a.service.ListRepos(ctx)
	return nil, reposOutput{Repos: repos}, err
}

func (a *API) repoGet(ctx context.Context, _ *mcp.CallToolRequest, input repoIDInput) (*mcp.CallToolResult, domain.Repo, error) {
	repo, err := a.service.GetRepo(ctx, input.RepoID)
	return nil, repo, err
}

func (a *API) repoSync(ctx context.Context, _ *mcp.CallToolRequest, input repoIDInput) (*mcp.CallToolResult, domain.Repo, error) {
	repo, err := a.service.SyncRepo(ctx, input.RepoID)
	return nil, repo, err
}

func (a *API) repoRemove(ctx context.Context, _ *mcp.CallToolRequest, input repoIDInput) (*mcp.CallToolResult, deletedOutput, error) {
	err := a.service.RemoveRepo(ctx, input.RepoID)
	return nil, deletedOutput{Deleted: err == nil, ID: input.RepoID}, err
}

func (a *API) anchorCreate(ctx context.Context, _ *mcp.CallToolRequest, input createAnchorInput) (*mcp.CallToolResult, domain.Anchor, error) {
	anchor, err := a.service.CreateAnchor(ctx, app.CreateAnchorInput{
		RepoID:    input.RepoID,
		Ref:       input.Ref,
		Path:      input.Path,
...
func (a *API) anchorClose(ctx context.Context, _ *mcp.CallToolRequest, input anchorIDInput) (*mcp.CallToolResult, domain.Anchor, error) {
	anchor, err := a.service.CloseAnchor(ctx, input.AnchorID)
	return nil, anchor, err
}

func (a *API) anchorReopen(ctx context.Context, _ *mcp.CallToolRequest, input anchorIDInput) (*mcp.CallToolResult, domain.Anchor, error) {
	anchor, err := a.service.ReopenAnchor(ctx, input.AnchorID)
	return nil, anchor, err
}

func (a *API) anchorResolve(ctx context.Context, _ *mcp.CallToolRequest, input anchorIDInput) (*mcp.CallToolResult, domain.Anchor, error) {
	anchor, err := a.service.ResolveAnchor(ctx, input.AnchorID)
	return nil, anchor, err
}

func (a *API) anchorComment(ctx context.Context, _ *mcp.CallToolRequest, input commentInput) (*mcp.CallToolResult, domain.Comment, error) {
	comment, err := a.service.CreateComment(ctx, input.AnchorID, input.ParentID, input.Author, input.Body)
	return nil, comment, err
}

func (a *API) anchorSearch(ctx context.Context, _ *mcp.CallToolRequest, input searchInput) (*mcp.CallToolResult, searchOutput, error) {
...
func (a *API) anchorGet(ctx context.Context, _ *mcp.CallToolRequest, input anchorIDInput) (*mcp.CallToolResult, domain.Anchor, error) {
	anchor, err := a.service.GetAnchor(ctx, input.AnchorID)
	return nil, anchor, err
}

func (a *API) anchorComments(ctx context.Context, _ *mcp.CallToolRequest, input anchorIDInput) (*mcp.CallToolResult, commentsOutput, error) {
	comments, err := a.service.ListComments(ctx, input.AnchorID)
	return nil, commentsOutput{Comments: comments}, err
}

func (a *API) anchorEvents(ctx context.Context, _ *mcp.CallToolRequest, input anchorIDInput) (*mcp.CallToolResult, eventsOutput, error) {
	events, err := a.service.ListAnchorEvents(ctx, input.AnchorID)
	return nil, eventsOutput{Events: events}, err
}

func (a *API) anchorStale(ctx context.Context, _ *mcp.CallToolRequest, input staleInput) (*mcp.CallToolResult, staleOutput, error) {
	entries, err := a.service.StaleQueue(ctx, input.RepoID, input.Limit)
	return nil, staleOutput{Stale: entries}, err
}
...
func (a *API) anchorFileView(ctx context.Context, _ *mcp.CallToolRequest, input fileViewInput) (*mcp.CallToolResult, app.FileView, error) {
	view, err := a.service.FileView(ctx, input.RepoID, input.Ref, input.Path)
	return nil, view, err
}
...
func (a *API) readEvents(ctx context.Context, request *mcp.ReadResourceRequest) (*mcp.ReadResourceResult, error) {
	parsed, err := url.Parse(request.Params.URI)
	if err != nil {
		return nil, err
	}
	anchorID := strings.TrimPrefix(parsed.Path, "/")
	events, err := a.service.ListAnchorEvents(ctx, anchorID)
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			return nil, mcp.ResourceNotFoundError(request.Params.URI)
		}
	...
	return jsonResource(request.Params.URI, events)
}
